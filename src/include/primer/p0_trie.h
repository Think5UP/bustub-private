...
class TrieNode {
 public:
   ...
   */
  TrieNode(TrieNode &&other_trie_node) noexcept : key_char_(other_trie_node.key_char_) {
  ...
  bool Insert(const std::string &key, T value) {
    ...
    while (word != key.end()) {
      ...
      if (!child->get()->HasChild(*cur)) {
        child = child->get()->InsertChildNode(*cur, std::make_unique<TrieNode>(*cur));
      } else {
        // 如果有那就返回它本身
    ...
    child = child->get()->InsertChildNode(*word, std::make_unique<TrieNode>(*word));
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    while (c != key.end()) {
      ...
      if (next_child->get()->IsEndNode() && c == key.end()) {
        auto flag_node = dynamic_cast<TrieNodeWithValue<T> *>(next_child->get());
        if (!flag_node) {
          *success = false;
          break;
        }
